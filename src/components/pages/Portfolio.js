...
const description1 = "I worked in a team of four members to create an application that assists a user locate private driveways that may be rented by the hour. The application uses the following technologies [1] React for the front end [2] Has a GraphQL API with a Node.js and Express.js server [3] Uses queries and mutations for retrieving, adding, updating, and deleting data [4] Uses MongoDB and the Mongoose ODM for the database and protects sensitive API key information on the server [5] Includes user authentication using JWT and [6] It is set up as a PWA ";
const description2 = "In this project, I started with starter code for a fully functioning Google Books API search engine built with a RESTful API, and refactored it to be a GraphQL API built with an Apollo Server. To complete the project I [1] Set up an Apollo Server to use GraphQL queries and mutations to fetch and modify data [2] Modify the existing authentication middleware so that it works in the context of a GraphQL API [3] Deploy the application to Heroku with a MongoDB database using MongoDB Atlas";
